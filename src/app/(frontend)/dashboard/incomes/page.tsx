...
export default async function IncomesPage({
  ...
  const [{ docs: allDocs }, { docs: pageDocs, totalDocs, totalPages }, { docs: catDocs }] =
    await Promise.all([
      payload.find({ collection: 'incomes', where, limit: 10000, depth: 0, sort: '-date' }),
      payload.find({
        collection: 'incomes',
        where,
        limit: PAGE_SIZE,
  ...
  const all = allDocs as unknown as IncomeDoc[]
  const rows = pageDocs as unknown as IncomeDoc[]
  ...
  const sum = (docs: IncomeDoc[], field: keyof F) =>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { label: 'Total Net', value: totals.net, text: 'text-emerald-700', ring: 'ring-emerald-100' },
          { label: 'Total VAT', value: totals.vat, text: 'text-slate-700', ring: 'ring-slate-200' },
          { label: 'Total Gross', value: totals.gross, text: 'text-emerald-800', ring: 'ring-emerald-200' },
      ...
      <div className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
        <div className="border-b border-slate-100 px-4 py-4 sm:px-6">
          <h2 className="text-sm font-semibold text-slate-900">All Income Entries</h2>
        </div>

        {/* Filters */}
        <Suspense>
          <TransactionFilters categories={categories} basePath="/dashboard/incomes" />
        </Suspense>

        {totalDocs === 0 ? (
          <div className="px-6 py-16 text-center">
            <p className="text-sm text-slate-400">
              {q || categoryParam ? 'No records match your filters.' : 'No income records yet.'}
            </p>
            {!q && !categoryParam && (
              <Link href="/dashboard/incomes/new" className="mt-3 inline-block text-sm font-medium text-indigo-600 hover:underline">
              ...
              <table className="min-w-full divide-y divide-slate-100 text-sm">
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      <tr key={row.id} className="transition-colors hover:bg-slate-50/60">
                        ...
                        <td className="whitespace-nowrap px-4 py-3 font-semibold tabular-nums text-emerald-700">{fmt(row.totalGross)}</td>
                        ...
                        <td className="whitespace-nowrap py-3 pl-2 pr-6">
                          <Link
                            href={`/dashboard/incomes/${row.id}/edit`}
                ...
                <tfoot className="border-t-2 border-slate-200 bg-slate-50">
                  <tr>
                    ...
                    <td className="whitespace-nowrap px-4 py-3 font-bold tabular-nums text-emerald-700">{fmt(totals.gross)}</td>
                    <td className="pr-6" />
                  </tr>
                </tfoot>
              </table>
